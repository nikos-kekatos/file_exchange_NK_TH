...
//        toEulerAngle(odom_.pose.pose.orientation, roll, pitch, yaw);
        ...
        x = 0.1; //odom_.pose.pose.position.x;
        assert(x>=0);
        assert(x<=0.2);
        
        y = 0.1; //odom_.pose.pose.position.y;
        z = 0.1; //odom_.pose.pose.position.z;
        
        
        vx = 0.01; //odom_.twist.twist.linear.x;
        vy = 0.01; //odom_.twist.twist.linear.y;
        vz = 0.01; //odom_.twist.twist.linear.z;
        
        VectorXf target(NO_OF_STATES);
        target(0) = 0.2; //pose_.pose.position.x;
        target(1) = 0.2; //pose_.pose.position.y;
        target(2) = 0.0;
